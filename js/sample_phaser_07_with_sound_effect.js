// Sample for phaser.js by T. Fujita on 2019/9/20 (for Maze)
...
class Scene_GameStart extends Phaser.Scene {
  constructor (){
    ...
    preload ()
    {
        ...
        this.load.image('Goal', './images/Goal_02.png');
...
class Scene_NextGame extends Phaser.Scene {
    ...
    create(){
	let sceneName01 = this.add.text(pos.length * P_size / 2, pos[0].length * P_size / 2 - 150, 'Goal !').setFontSize(40).setFontFamily("Arial").setOrigin(0.5).setColor("#ff0000").setInteractive();
	let sceneName02 = this.add.text(pos.length * P_size / 2, pos[0].length * P_size / 2 - 100, 'Time = ' + t_time + ' Sec.').setFontSize(30).setFontFamily("Arial").setOrigin(0.5).setColor("#dd0000").setInteractive();
	let sceneName03 = this.add.text(400, 300, 'ここで使用しているグラフィクスの一部に「どらぴか」様 \n URL: https://dorapika.wixsite.com/pikasgame 作成の素材を\nまた、サウンドは「soundeffect-lab　https://soundeffect-lab.info/ 」様\n作成の素材を使用しております。').setFontSize(20).setFontFamily("Arial").setOrigin(0.5).setColor("#FF0000").setInteractive();

	let change = this.add.text(400, 450, 'Push Here for Next Game').setFontSize(30).setFontFamily("Arial").setColor("#FF0000").setOrigin(0.5).setInteractive();
